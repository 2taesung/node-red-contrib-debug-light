module.exports = function (RED) {
  ...
  function DebugLightNode(n) {
    ...
    this.active =
      n.active === null || typeof n.active === "undefined" || n.active;
    if (this.tostatus) {
    ...
    if (editExpression) {
      try {
        preparedEditExpression = RED.util.prepareJSONataExpression(
          editExpression,
          this
        );
      } catch (e) {
    ...
    if (statExpression) {
      try {
        preparedStatExpression = RED.util.prepareJSONataExpression(
          statExpression,
          this
        );
      } catch (e) {
    ...
    function prepareStatus(msg, done) {
      if (node.statusType === "auto") {
        ...
        if (preparedStatExpression) {
          ...
          var output;
          try {
            output = RED.util.getMessageProperty(msg, node.statusVal);
          } catch (err) {
            output = undefined;
          }
    ...
    this.on("input", function (msg, send, done) {
      if (
        hasOwnProperty.call(msg, "status") &&
        hasOwnProperty.call(msg.status, "source") &&
        hasOwnProperty.call(msg.status.source, "id") &&
        ...
        if (node.statusType === "counter") {
          ...
          prepareStatus(msg, function (err, debugMsg) {
            ...
            if (
              ...
              hasOwnProperty.call(output, "fill") &&
              hasOwnProperty.call(output, "shape") &&
              hasOwnProperty.call(output, "text")
            ) {
              fill = output.fill;
              shape = output.shape;
              st = output.text;
            ...
            if (JSON.stringify(newStatus) !== node.oldState) {
              // only send if we have to
  ...
  RED.nodes.registerType("debug-light", DebugLightNode, {
    settings: {
      // debugUseColors: {
      //   value: false,
      // },
  ...
  DebugLightNode.logHandler = new events.EventEmitter();
  DebugLightNode.logHandler.on("log", function (msg) {
    if (msg.level === RED.log.WARN || msg.level === RED.log.ERROR) {
      sendDebug(msg);
    }
  });
  RED.log.addHandler(DebugLightNode.logHandler);
  ...
  RED.httpAdmin.get("/debug/view/view.html", function (req, res) {
    if (!cachedDebugView) {
      fs.readFile(path.join(__dirname, "lib", "debug", "view.html"))
        .then((data) => {
          let customStyles = "";
          try {
          ...
          cachedDebugView = data
            .toString()
            .replace("<!-- INSERT-THEME-CSS -->", customStyles);
